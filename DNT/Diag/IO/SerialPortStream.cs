...
namespace System.IO.Ports
{
    class SerialPortStream : Stream, ISerialStream, IDisposable
    {
        ...
        [DllImport("dntdiag", SetLastError = true)]
...
// buffer (not the SerialPort class buffer)
        ...
        public override int Read([In, Out] byte[] buffer, int offset, int count)
        {
            ...
            #if TOMIC_DEVICE_V1
            long expire = Timer.FromMilliseconds(ReadTimeout).Ticks;
            Stopwatch watch = Stopwatch.StartNew();

            int len = 0;

            while (watch.ElapsedTicks < expire)
            {
                int size = BytesToRead;
                if (size >= count)
                {
                    size = read_serial(fd, buffer, offset, count);
                    if (size == -1)
                        ThrowIOException();
                    len += size;
                    return len;
                ...
                if (size != 0)
                {
                    size = read_serial(fd, buffer, offset, size);
                    if (size == -1)
                        ThrowIOException();
                    len += size;
                    count -= size;
